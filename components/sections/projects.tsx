...
import askRoomPrintSCreen from "@/assets/images/ask-room.png";
      ...
      <div className="container mx-auto">
        ...
        <div className="flex flex-col gap-20 mt-10 md:mt-20">
          {portfolioProjects.map(
            (
              { title, tag, year, topics, demo, repository, image, wip },
              index,
            ) => (
              <Card
                key={title}
              ...
              >
                <div className="lg:grid lg:grid-cols-2 lg:gap-16">
                  <div className="lg:pb-16 relative">
                    ...
                    {wip && (
                      <span className="p-1 absolute top-0 right-0 bg-card border-white/33 border-1 rounded-full text-xs">
                        🔨 Em construção
                      </span>
                    )}
                    ...
                    {wip && (
                      <button
                        type="button"
                        className="bg-white text-gray-950 h-11 md:h-12 w-full px-6 md:w-auto rounded-xl font-semibold inline-flex items-center justify-center gap-2 mt-4 md:mt-8 cursor-not-allowed opacity-50"
                        disabled
                      >
                        <span>Demo em breve</span>
                        <ArrowUpRight className="size-4" />
                      </button>
                    )}
                  </div>
